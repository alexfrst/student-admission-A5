...
class SatSolver():
    ...
    def _clauses_to_dimacs(self, clauses, numvar):
        ...
        @param clauses: {List(List(Int))} Clauses générées précedemment.
    ...
    def _solve(self, clauses, variables, i2variables, filename, cmd="./gophersat/gophersat-1.1.6.exe"):
        ...
        self._write_dimacs_file(myDimacs, "workingfile.cnf")

        result = subprocess.run([cmd, filename], stdout=subprocess.PIPE, check=True, encoding="utf8")
        ...
        return True, [int(x) for x in model[:-1] if int(x) != 0], {i2variables[abs(int(v))]: int(v) > 0 for v in model
                                                                   if int(v) != 0}
    ...
    def solve(self, S, y):
        ...
        clauses = self._generate_clause1(**params1) + self._generate_clause2(**params1) + self._generate_clause3(
            **params2) + self._generate_clause4(**params3) + self._generate_clause5(
            **{"subjects_count": subjects_count, **params3})

        return self._solve(clauses, variables, i2variables, "workingfile.cnf")
